...
impl Loader {
    ...
    pub fn load_gguf_info(
        ...
        cursor
            ...
            .map_err(|e| LociError::IoWithContext {
        ...
        let metadata_kv_count =
            cursor
                .read_u64::<LittleEndian>()
                .map_err(|e| LociError::IoWithContext {
                    context: "reading metadata_kv_count",
                    source: e,
                })?;
        ...
        for _ in 0..gguf_headers.metadata_kv_count {
            ...
            let value_type =
                Self::read_gguf_type(&mut cursor).map_err(|e| LociError::InvalidFileMetadata {
                    file_type: "GGUF".into(),
        ...
        for _ in 0..gguf_headers.tensor_count {
            ...
            let n_dims =
                Self::read_gguf_int32(&mut cursor).map_err(|e| LociError::InvalidFileMetadata {
                    file_type: "GGUF".into(),
            ...
            let offset =
                Self::read_gguf_int64(&mut cursor).map_err(|e| LociError::InvalidFileMetadata {
                    file_type: "GGUF".into(),
    ...
    fn read_gguf_uint8(cursor: &mut Cursor<&[u8]>) -> Result<u8, LociError> {
        cursor.read_u8().map_err(|e| LociError::IoWithContext {
            context: "reading u8",
            source: e,
        })
    }
    ...
    fn read_gguf_int8(cursor: &mut Cursor<&[u8]>) -> Result<i8, LociError> {
        cursor.read_i8().map_err(|e| LociError::IoWithContext {
            context: "reading i8",
            source: e,
        })
    }
    ...
    fn read_gguf_bool(cursor: &mut Cursor<&[u8]>) -> Result<bool, LociError> {
        let value = cursor.read_i8().map_err(|e| LociError::IoWithContext {
            ...
            GgufType::String => {
                let len =
                    cursor
                        .read_u64::<LittleEndian>()
                        .map_err(|e| LociError::IoWithContext {
                            context: "reading string length for skip",
                            source: e,
                        })?;
                cursor.seek(SeekFrom::Current(len as i64)).map_err(|e| {
                    LociError::IoWithContext {
                        context: "skipping string",
                        source: e,
                    }
                })?;
            }
            GgufType::Array => {
                ...
                let entries_num =
                    cursor
                        .read_u64::<LittleEndian>()
                        .map_err(|e| LociError::IoWithContext {
                            context: "reading array length for skip",
                            source: e,
                        })?;
                for _ in 0..entries_num {

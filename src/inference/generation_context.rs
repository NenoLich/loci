use crate::inference::{ModelCacheManager, CacheMetadata};
use crate::inference::model_cache::load_mixed_cache;
use crate::model::{MixedCache, ModelCacheType, ModelCacheInfo};
...
impl GenerationContext {
    pub fn new(model: &str, model_cache_config: Option<ModelCacheConfig>, cache_info: ModelCacheInfo) -> Result<Self, LociError> {
        ...
        let model_cache_fragmentation = match model_cache_config.fragmentation {
            ModelCacheFragmentation::BlockWise { block_size } => {
                ModelCacheFragmentation::BlockWise { block_size: block_size.max(cache_info.cache_block_size_hint) }
            }
            ModelCacheFragmentation::TokenWise => {
                if cache_info.cache_block_size_hint > 1 {
                    ModelCacheFragmentation::BlockWise { block_size: cache_info.cache_block_size_hint }
                } else {
                    ModelCacheFragmentation::TokenWise
                }
            }
        };
        let cache_manager = ModelCacheManager::builder(model.clone())
    ...
    pub fn update(&mut self, input_tokens:Vec<u32>) -> Result<(), LociError> {
        ...
        if self.active_cache.len() != 0 && let ModelCacheType::MixedWithConv { conv_l_cache } = self.cache_type {
    ...
    fn update_conv_cache(&mut self, input_tokens_len: usize, conv_l_cache: usize) -> Result<(), LociError> {
        if self.prefix_caching && let ModelCacheFragmentation::BlockWise { block_size } = self.model_cache_fragmentation {
            ...
            if stored_block_count < estimated_block_count {
                ...
                let block_boundary_input_indices = (stored_block_count..estimated_block_count)
                    .map(|count| (count - stored_block_count, ((count + 1) * block_size) - added_token_id_len))
                    .collect::<Vec<(usize, usize)>>();
                let mut new_block_boundary_conv_cache = vec![vec![None; self.model_layers_count]; new_block_count];

                for (i, layer_cache) in self.active_cache.iter_mut().enumerate() {
                    if let Some(MixedCache::ConvCache(tensor)) = layer_cache {
                        let tensor_len = tensor.dim(self.model_cache_seq_len_dim)
                        ...
                        for (block_idx, block_boundary_idx) in &block_boundary_input_indices {
                            ...
                            let boundary_cache = tensor.narrow(self.model_cache_seq_len_dim, boundary_start, conv_l_cache)
                                .map_err(|e| LociError::Cache(e.to_string()))?;
                            new_block_boundary_conv_cache[*block_idx][i] = Some(MixedCache::ConvCache(boundary_cache));
                        }
                        let trim_start = tensor_len.saturating_sub(conv_l_cache);
                        *tensor = tensor.narrow(self.model_cache_seq_len_dim, trim_start, conv_l_cache)
                            .map_err(|e| LociError::Cache(e.to_string()))?;
                    }
                }
                self.block_boundary_conv_cache.extend(new_block_boundary_conv_cache);
            } else {
    ...
    pub fn match_cache(&mut self, prompt_token_ids: &[u32], min_prefill_tokens: usize, device: &Device, conv_on_cpu: bool) -> Result<usize, LociError> {
        ...
        let mut highest_cache_match_len = self.token_ids.iter()
        ...
        for cache_metadata in &self.cache_metadata {
            let cache_match_len = cache_metadata.token_ids.iter()
                ...
                .take_while(|(a, b)| a == b)
                .count();
            if cache_match_len > highest_cache_match_len && cache_match_len >= self.cache_manager.min_cache_tokens {
        ...
        let result = match (highest_cache_match_len, active_cache_source, cache_file_path_with_highest_match) {
            ...
            (_, false, Some(cache_file_path)) => {
                match load_mixed_cache(cache_file_path, cache_len_with_highest_match, highest_cache_match_len, &self.model_cache_fragmentation, self.model_cache_seq_len_dim, device, conv_on_cpu)
                    .map_err(|e| LociError::CacheLoad(format!("failed to load cache: {}", e))) {
                        Ok(loaded_mixed_cache) => {
                            debug!("Cache loaded from disk");
                            debug!("Matched cache length: {}", loaded_mixed_cache.cached_token_length);
                            self.save_active_cache();
                            self.token_ids.clear();
                            self.token_ids.extend_from_slice(&prompt_token_ids[..loaded_mixed_cache.cached_token_length]);
                            self.active_cache = loaded_mixed_cache.mixed_cache;
                            self.block_boundary_conv_cache = loaded_mixed_cache.block_boundary_conv_cache;
        ...
        for (layer_idx, cache) in self.active_cache.iter_mut().enumerate() {
            if let Some(MixedCache::KvCache(concat_kv_cache)) = cache {
                if concat_kv_cache.k().is_some() && concat_kv_cache.v().is_some() {
                    ...
                    *k = k.narrow(self.model_cache_seq_len_dim, 0, matched_cache_length)
                        .map_err(|e| LociError::CacheLoad(e.to_string()))?;
                    let v = concat_kv_cache.v_mut().unwrap();
                    *v = v.narrow(self.model_cache_seq_len_dim, 0, matched_cache_length)
                        .map_err(|e| LociError::CacheLoad(e.to_string()))?;
                }
                continue;
            }
            if let Some(MixedCache::ConvCache(conv_cache)) = cache {
                if let Some(idx) = block_idx {
                    let block_boundary_cache = if let Some(MixedCache::ConvCache(tensor)) = &self.block_boundary_conv_cache[idx][layer_idx] {
                        Ok(tensor.clone())
                    } else {
                        Err(LociError::Cache("block boundary cache not found".to_string()))
                    }?;
    ...
    pub fn reset_active_cache(&mut self, new_cache: Vec<Option<MixedCache>>, with_save: bool) -> Result<(), LociError> {
        if with_save {
            self.save_active_cache();
        }
        self.token_ids.clear();
    ...
    pub fn save_active_cache(&mut self) {
        ...
        let error = match self.cache_manager.save_cache(&self.token_ids, &self.active_cache, self.model_cache_seq_len_dim, &self.model_cache_fragmentation, &self.block_boundary_conv_cache) {
            Ok(cache_file_path) => {
                ...
                self.cache_manager.enforce_limits(&mut self.cache_metadata).err()
            }
            Err(e) => Some(e),
        };

...
use crate::{
    ...
    inference::{InferenceEngine, GenerationReport},
...
pub async fn chat_completions(
    ...
    if state.command_tx.send(WorkerCommand::ChatCompletion { req, response_tx }).await.is_err() {
    ...
    match response_rx.await {
        ...
        Ok(Err(server_error)) => {
            let error = json!({"error": {"message": server_error, "type": "server_error", "code": 500}});
